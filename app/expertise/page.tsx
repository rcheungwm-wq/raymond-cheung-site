  ...
  return (
    <div style={{ backgroundColor: "var(--warm-ivory)", paddingTop: "96px" }}>
      ...
      {expertiseAreas.map((area, i) => (
        <section key={area.id} id={area.id} aria-labelledby={`expertise-${area.id}`} style={{ backgroundColor: i % 2 === 0 ? "var(--warm-ivory)" : "var(--soft-mist)", padding: "5rem 2rem", borderBottom: "1px solid rgba(7,26,43,0.06)" }}>
          <div style={{ maxWidth: "1280px", margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "5rem", alignItems: "start" }} className="expertise-detail-grid">
            <div>
              ...
              <h2 id={`expertise-${area.id}`} style={{ fontFamily: "var(--font-plus-jakarta), system-ui, sans-serif", fontSize: "clamp(1.4rem, 2.5vw, 2rem)", fontWeight: 700, letterSpacing: "-0.025em", color: "var(--midnight-navy)", marginBottom: "1rem", lineHeight: 1.2 }}>
                {area.title}
              </h2>
              ...
              <div style={{ marginBottom: "2rem" }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem" }}>
                  {area.topics.map((topic) => (
                    <span key={topic} style={{ fontFamily: "var(--font-ibm-mono), monospace", fontSize: "0.62rem", letterSpacing: "0.06em", color: "var(--graphite)", backgroundColor: "var(--white)", border: "1px solid rgba(7,26,43,0.1)", padding: "0.3rem 0.65rem", borderRadius: "1px" }}>{topic}</span>
                  ))}
                </div>
              </div>
            ...
            <div>
              <div style={{ backgroundColor: "var(--white)", border: "1px solid rgba(7,26,43,0.08)", padding: "2rem", marginBottom: "1.5px" }}>
              ...
              <div style={{ backgroundColor: "var(--white)", border: "1px solid rgba(7,26,43,0.08)", padding: "2rem", marginBottom: "1.5px" }}>
                ...
                <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
                  {area.questions.map((q) => (
                    <li key={q} style={{ fontSize: "0.875rem", color: "var(--midnight-navy)", padding: "0.6rem 0", borderBottom: "1px solid rgba(7,26,43,0.05)", display: "flex", alignItems: "flex-start", gap: "0.6rem", lineHeight: 1.5 }}>
              ...
              <div style={{ backgroundColor: "var(--white)", border: "1px solid rgba(7,26,43,0.08)", padding: "2rem" }}>
